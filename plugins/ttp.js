...
let handler = async (m, { conn, text, command }) => {
  ...
  if (/^ttp1?$/i.test(command)) {
    let stiker = await sticker(null, global.API('lol', '/api/ttp', { text: teks }, 'apikey'), global.packname, global.author)
  ...
  if (/3$/i.test(command)) {
    let stiker = await sticker(null, global.API('lol', '/api/ttp3', { text: teks }, 'apikey'), global.packname, global.author)
    if (stiker) return await conn.sendMessage(m.chat, stiker, MessageType.sticker, {
      quoted: m
    })
    throw stiker.toString()
  ...
  if (/2$/i.test(command)) {
    let stiker = await sticker(null, global.API('lol', '/api/ttp2', { text: teks }, 'apikey'), global.packname, global.author)
    if (stiker) return await conn.sendMessage(m.chat, stiker, MessageType.sticker, {
      quoted: m
    })
    throw stiker.toString()
